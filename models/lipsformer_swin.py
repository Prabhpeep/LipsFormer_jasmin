...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.alpha1 = ScaleLayer(dim=dim, alpha=1.0/num_layers) 
        self.alpha2 = ScaleLayer(dim=dim, alpha=1.0/num_layers) 
    ...
    def forward(self, x):
        ...
        i = 0
        if len(self.dwconv_layers) > 0 :
            for deconv, act, scale in zip(self.dwconv_layers, self.gelus, self.scale_layers):
                tmp_x = x
    ...
    def flops(self):
        ...
        flops += self.dim * H * W
        flops += self.dim * H * W * 49
        flops += self.dim * H * W * 49
        flops += self.dim * H * W * self.dim
        flops += self.dim * H * W
        flops += self.dim * H * W
        return flops
...
class LipsFormerSwin(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               use_checkpoint=use_checkpoint, num_layers=LL)
